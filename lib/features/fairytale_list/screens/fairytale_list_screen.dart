...
class _SharedFairytaleGridState extends State<_SharedFairytaleGrid> {
  ...
  Future<void> _open(MyFairytale item) async {
    ...
    if (offline != null) {
      ...
      await Navigator.of(context).push(
        MaterialPageRoute(
          // 저장본으로 열더라도 남의 공유 동화이므로 신고할 수 있어야 한다.
          // 이 분기는 연결 상태와 무관하게 저장본만 있으면 발동하므로, 여기서
          // 빼면 한 번 다운로드한 동화는 온라인에서도 영영 신고할 수 없게 된다.
          builder: (_) => FairytaleSlideScreen(
            fairytale: response,
            lang: item.language,
            voiceType: 'dad',
            reportFairytaleId: item.id,
            reportOwnerId: item.ownerId,
          ),
        ),
      );
      return;
